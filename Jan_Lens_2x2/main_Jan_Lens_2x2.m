...
disp('=======================================');
disp('Initial Generation Simulation Complete');
disp('=======================================');
...
while(x<=totalGenerations)
    ...
    bestCostByGeneration(x,1)=x;
    [bestCostByGeneration(x,2),temp]=min(finalCosts);
    bestLensGeom(:,:,x)=nextGen(:,:,temp(1));
    ...
    figure(1)
    plot(bestCostByGeneration(:,1),bestCostByGeneration(:,2));
    xlabel('Generations');
    ylabel('Cost');
    
    figure(2)
    drawLens(bestLensGeom(:,:,x),unitWidth,portLocations);
%     catch err
%         disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
...
figure(1)
plot(bestLensGeom(:,1), bestLensGeom(:,2));
xlabel('Generations');
ylabel('Cost');
