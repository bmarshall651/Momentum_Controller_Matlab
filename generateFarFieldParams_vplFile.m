function [ outputVplFile ] = generateFarFieldParams_vpl( frequency, portLocations, portExcitationNumber, PortImpedances, outputVplFile)
...
while(x<=numOfPorts)
    ...
    fprintf(fOut, ['AMPLITUDE ',num2str(sqrt(real(PortImpedances)^2+imag(PortImpedances)^2)),' PHASE ',atan(imag(PortImpedances)/real(portImpedances)),'; \n']);
    x=x+1;
end
